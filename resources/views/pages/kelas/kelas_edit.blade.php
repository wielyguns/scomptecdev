...
<div class="box-content">
	...
	<section class="content-wrap">
		<div class="form-wrap row">
			<div class="col-8">
				<form method="post" action="{{ route('kelas_update', $data->id) }}">
					{{ csrf_field() }}
					{{ method_field('PUT') }}
					...
					<div class="form-group row">
						...
						<div class="col-8">
							<div class="custom-control custom-radio custom-control-inline">
								<input class="custom-control-input" id="jenis1" type="radio" name="jenis" value="Reguler" {{ $data->jenis == "Reguler" ? "checked='true'":"" }}>
								<label class="custom-control-label" for="jenis1">Reguler</label>
							</div>
							<div class="custom-control custom-radio custom-control-inline">
								<input class="custom-control-input" id="jenis2" type="radio" name="jenis" value="Private" {{ $data->jenis == "Private" ? "checked='true'":"" }}>
					...
					<div class="form-group row">
						...
						<div class="col-8">
							<select id="program_kursus_id" class="select-control {{ $errors->has('program_kursus_id') != "" ? "invalid" : "" }}" title="Pilih program kursus" data-live-search="true" name="program_kursus_id">
								...
								<option value="{{ $p->id }}" data-kode="{{ $p->kode }}" {{ ($data->program_kursus_id == $p->id ? "selected":"") }}>{{ $p->nama }}</option>
								@endforeach()
							</select>
					...
					<div class="form-group row">
						...
						<div class="col-8">
							...
							<input id="kode" type="hidden" name="kode" value="{{ $data->kode }}">
							@if($errors->has('kode'))
					...
					<div class="form-group row">
						...
						<div class="col-8">
							<div class="input-group {{ $errors->has('durasi') != "" ? "invalid" : "" }}">
								<input class="form-control number" type="text" name="durasi" value="{{ $data->durasi }}">
					...
					<div class="form-group row">
						...
						<div class="col-8">
							<div class="input-group {{ $errors->has('kapasitas') != "" ? "invalid" : "" }}" id="kapasitas">
								<input class="form-control number" type="text" name="kapasitas" value="{{ $data->kapasitas }}">
					...
					<div class="form-group row">
						...
						<div class="col-8">
							<div class="input-group {{ $errors->has('biaya') != "" ? "invalid" : "" }}">
								...
								<input class="form-control number rupiah" type="text" name="biaya" value="{{ $data->biaya }}">
							</div>
							@if($errors->has('biaya'))
...
<script type="text/javascript">
	...
	if($('input[name=jenis]:checked').val() == "Private") {
		$('#kapasitas').closest('.form-group').hide();
	}
